...
app.controller('HomeCtrl',function($scope,$rootScope,$http,RESTClient,Names,$state,URL)
{
    ...
    $scope.select=function()
    {
        ...
        .then(function(data1)
        {
            ...
            for(var i=0;i<prelink.length;i++)
            {
                if(prelink[i].name==name)continue;
                ...
                for(var j=0;j<prelink[i].links.length;j++)
                {
                    if(prelink[i].links[j]==name)continue;
                    var node={"category":2, "name": prelink[i].links[j]+'.', "value" : 20, label:prelink[i].links[j]};
                    var link={"source" :prelink[i].name+'.', "target" :prelink[i].links[j]+'.', "weight" :j*10+i*5, "name": "转发了TA"};
                    input_nodes.push(node);
                    input_links.push(link);
